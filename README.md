...
>    护栏**能改动评级**的规则有两条——规则 2「`AML-001` 强制 HIGH」与
>    规则 4「无依据兜底保守升级」——但在评级集上只有规则 2 会生效：
>    规则 4 要求 `hold` 为真，而集里只有 AML-001 那一条触发 `hold`，
>    等级那时已被规则 2 抬到 HIGH。（规则 4 确实改过评级，改在集外——
>    注入样例 `I001` 的 LOW→HIGH 就是它做的。）想要更高的纠正数，得扩案例集，
